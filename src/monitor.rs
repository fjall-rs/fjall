...
impl Monitor {
    ...
    pub fn run(&self) -> bool {
        ...
        if size as f64 > (self.keyspace_config.max_journaling_size_in_bytes as f64 * 0.5) {
            ...
            if seqno_map.is_empty() {
                ...
                let actual_seqno_map = seqno_map
                    .iter()
                    .map(|(key, (_, seqno, _, _))| (key.clone(), seqno.clone()))
                    .collect::<HashMap<_, _>>();

                if actual_seqno_map
                    .keys()
                    .any(|name| !partitions_names_with_queued_tasks.contains(name))
                {
                    if journal_manager
                        ...
                        .is_ok()
                    {
                        ...
                        for (name, (partition, _, yanked_id, yanked_memtable)) in seqno_map {
        ...
        // TODO: it would be better to rotate the journal once
        // and then add all memtables of partitions to flush queue
        // so not every memtable rotation is creating a new journal
